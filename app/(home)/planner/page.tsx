...
type DragItem = DraggedProduct | DraggedBundle;

// Extend window interface for calculateDropPosition
declare global {
  interface Window {
...
const DropPositionCalculator = () => {
  ...
  React.useEffect(() => {
    ...
    return () => {
      delete window.calculateDropPosition;
    };
  }, [camera, gl, calculateDropPosition]);
...
const CanvasWrapper = ({
  ...
  const lightsOn = useStore(
    (state: ReturnType<typeof useStore.getState>) => state.lightsOn
  );
  const addWardrobeInstance = useStore(
    (state: ReturnType<typeof useStore.getState>) => state.addWardrobeInstance
  );
  const updateWardrobeInstance = useStore(
    (state: ReturnType<typeof useStore.getState>) =>
      state.updateWardrobeInstance
  );
  const focusedWardrobeInstance = useStore(
    (state: ReturnType<typeof useStore.getState>) =>
      state.focusedWardrobeInstance
  );
  ...
  const [{ isOver }, drop] = useDrop<DragItem, void, { isOver: boolean }>({
    ...
    drop: (item: DragItem, monitor: DropTargetMonitor<DragItem>) => {
      ...
      if (clientOffset && window.calculateDropPosition) {
        const calculatedPosition = window.calculateDropPosition(
          clientOffset.x,
          clientOffset.y
        );
        if (calculatedPosition) {
          dropPosition = calculatedPosition;
        }
      }
      if ("product" in item) {
        ...
        if (focusedWardrobeInstance) {
          ...
          if (item.bundle.ItemName === "2583987-Original") {
            const originalProduct = products.find(
              (p: Product) => p.itemNumber === "2583987"
            );
            if (originalProduct) {
...
const AutoSaveIndicator = () => {
  ...
  const wardrobeInstances = useStore(
    (state: ReturnType<typeof useStore.getState>) => state.wardrobeInstances
  );
  const wallsDimensions = useStore(
    (state: ReturnType<typeof useStore.getState>) => state.wallsDimensions
  );
  const customizeMode = useStore(
    (state: ReturnType<typeof useStore.getState>) => state.customizeMode
  );
  const autoSaveEnabled = useStore(
    (state: ReturnType<typeof useStore.getState>) => state.autoSaveEnabled
  );
  ...
  useEffect(() => {
    ...
    // Use a timeout to avoid immediate state update
...
export default function Planner() {
  ...
  const setCanvasScreenshotDataUrl = useStore(
    (s: ReturnType<typeof useStore.getState>) => s.setCanvasScreenshotDataUrl
  );
  const setCurrentDesignCode = useStore(
    (s: ReturnType<typeof useStore.getState>) => s.setCurrentDesignCode
  );
  ...
  const focusedWardrobeInstance = useStore(
    (s: ReturnType<typeof useStore.getState>) => s.focusedWardrobeInstance
  );
  ...
  const loadSavedState = useStore(
    (state: ReturnType<typeof useStore.getState>) => state.loadSavedState
  );
  const resetToDefaultState = useStore(
    (state: ReturnType<typeof useStore.getState>) => state.resetToDefaultState
  );
  const setAutoSaveEnabled = useStore(
    (state: ReturnType<typeof useStore.getState>) => state.setAutoSaveEnabled
  );
  ...
  const totalPrice = wardrobeInstances.reduce(
    (sum: number, instance: { product: { price: number } }) => {
