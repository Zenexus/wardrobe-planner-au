...
type StoreState = {
  ...
  updateWardrobePositionWithHistory: (
    id: string,
    position: [number, number, number]
  ) => void;
...
export const useStore = create<StoreState>((set, get) => {
  ...
  return {
    ...
    // Color selection state
    selectedColor: "White",
    setSelectedColor: (selectedColor: "White" | "Oak") =>
      set({ selectedColor }),
    ...
    addWardrobeInstance: (
      ...
      if (forcePosition && position) {
        ...
        if (
          ...
          console.log(
            `Using forced position: [${position[0].toFixed(
              2
    ...
    // Special function for position updates when dragging ends - saves to history
    updateWardrobePositionWithHistory: (
      id: string,
      position: [number, number, number]
    ) => {
      // Save to history before the final position update
      get().saveToHistory();
      // Then update position normally
      get().updateWardrobePosition(id, position);
    },
    ...
    setWallDimensions: (
      ...
      // Save to history before changing wall dimensions
      get().saveToHistory();
    ...
    setFloorTexture: (texture: "wood" | "carpet" | "tile") => {
      // Save to history before changing floor texture
      get().saveToHistory();
      ...
      ([0, 1, 2, 3] as const).forEach((key) => {
        ...
        const effectiveGap =
          n > 1
            ? Math.max(
                0,
                Math.min(baseGap, (available - totalWidths) / (n - 1))
              )
            : 0;
    ...
    addOrganizer: (organizer: SelectedOrganizer) => {
      set((state) => {
        ...
        if (existingIndex >= 0) {
          ...
          // Add new organizer
          return {
            selectedOrganizers: [...state.selectedOrganizers, organizer],
          };
        }
      });
    },
    ...
    saveToHistory: () => {
      const state = get();
      const snapshot = {
        wardrobeInstances: JSON.parse(JSON.stringify(state.wardrobeInstances)),
        wallsDimensions: JSON.parse(JSON.stringify(state.wallsDimensions)),
    ...
    undo: () => {
      ...
      if (state.canUndo()) {
        ...
        set({
          ...
          wardrobeInstances: JSON.parse(
            JSON.stringify(snapshot.wardrobeInstances)
          ),
          wallsDimensions: JSON.parse(JSON.stringify(snapshot.wallsDimensions)),
          floorTexture: snapshot.floorTexture,
          // Reset UI states when undoing
          selectedObjectId: null,
          draggedObjectId: null,
          focusedWardrobeInstance: null,
          globalHasDragging: false,
